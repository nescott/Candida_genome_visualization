...
# Input vars 
spreadsheet_list <- "genome_sheets.txt"
sample_list <- "raxml_midpoint_phylogeny.csv"
...
## ---------------------------
# Concatenate snp counts and tidy data
...
p <- snp_again %>%
  mutate(clustered_samples = fct_relevel(sample, rev(sample_order$sample))) %>% 
    ...
    scale_fill_gradient(low = "white", high="#9C0824", na.value = "white", name="Heterozygous SNPs\n per 5kb window") +
    geom_tile() +
    theme_minimal() +
...
ggsave("MEC_Calbicans_LOH_heatmap.png", p, device=png, dpi=300, bg="white",
